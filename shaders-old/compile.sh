...
for file in ./*; do
    ...
    if [ "$extension" == "vs" ]; then
        ./shaderc.exe -f $file -o "$file"".bin" --type v --varyingdef varying.def.sc -i shaders/bgfx_shader.bsh
    fi
    if [ "$extension" == "fs" ]; then
        ./shaderc.exe -f $file -o "$file"".bin" --type f --varyingdef varying.def.sc -i shaders/bgfx_shader.bsh
    fi
done
